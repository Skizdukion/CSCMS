    ...
    try {
      ...
      if (searchFilters.sort_by_nearby && userLocation) {
        ...
        response = await storeApi.searchStores({
          ...
          radius_km: searchFilters.radius_km,
          page: page,
          limit: itemsPerPage
        });
      } else {
      ...
      if (response.success && response.data) {
        ...
        if (searchFilters.search || searchFilters.district || searchFilters.store_type || searchFilters.is_active !== undefined || searchFilters.inventory_item || searchFilters.sort_by_nearby || searchFilters.radius_km) {
          ...
          if (searchFilters.sort_by_nearby && userLocation) {
            searchMessage += ` sorted by distance`;
            if (searchFilters.radius_km) {
              searchMessage += ` within ${searchFilters.radius_km}km radius`;
            }
          } else if (searchFilters.radius_km && !searchFilters.sort_by_nearby) {
            searchMessage += ` (radius ${searchFilters.radius_km}km set but location search disabled)`;
          }
              ...
              <div className="filter-row">
                ...
                <div className="filter-group checkbox-group">
                  <label>
                    <input
                      type="checkbox"
                      checked={searchFilters.radius_km ? true : false}
                      onChange={(e) => {
                        if (e.target.checked) {
                      ...
                      disabled={gettingLocation}
                    />
                    Search within radius {gettingLocation ? '(Getting location...)' : ''}
                  </label>
                  {searchFilters.radius_km && (
                    <div className="radius-input-container">
                      <label htmlFor="radius-km-input">Radius (km):</label>
                      <input
                        type="number"
                        id="radius-km-input"
                        className="radius-km-input"
                        min="0.1"
                        max="50"
                        step="0.1"
                        value={searchFilters.radius_km}
                        onChange={(e) => {
                          const value = e.target.value ? parseFloat(e.target.value) : 5;
                          setSearchFilters(prev => ({ 
                            ...prev, 
                            radius_km: value
                          }));
        ...
        <div className="stores-grid">
          {stores.length > 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="store-info">
                    ...
                    {distance && (
                      <div className={`store-distance ${searchFilters.radius_km && distance <= searchFilters.radius_km ? 'within-radius' : ''}`}>
                        📍 {distance.toFixed(1)} km away
                        {searchFilters.radius_km && distance <= searchFilters.radius_km && (
                          <span className="radius-indicator"> ✓ Within {searchFilters.radius_km}km radius</span>
                        )}
                      </div>
                    )}
                  </div>
            ...
            <div className="no-stores">
              {(searchFilters.search || searchFilters.district || searchFilters.store_type || searchFilters.is_active !== undefined || searchFilters.inventory_item || searchFilters.sort_by_nearby || searchFilters.radius_km) ? (
                <div>
